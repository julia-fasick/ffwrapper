...
function build_cmd () {
    ...
    if [[ $TFLAG -eq 1 ]]; then
       ...
       if ! [[ "$SRC_BITRATE_RAW" =~ ^[0-9]+$ ]] || [[ "$SRC_BITRATE_RAW" -eq 0 ]]; then
        ...
        # use new values
        RET="${RET} -c:v hevc_nvenc -preset p7 -vtag hvc1 -profile:v main10 -tune:v hq -rc:v vbr -multipass 2 -b:v ${AUTO_BV}k -maxrate ${AUTO_MAXRATE}k -spatial-aq 1 -aq-strength 8 -rc-lookahead 32"
    ...
    # echo command for debug purposes. maybe move this to a log
    echo "$RET"
}
